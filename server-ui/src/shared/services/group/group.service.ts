...
export class GroupService {
  ...
  async create(label, contentSet, username): Promise<Group> {
    console.log("TODO: setup RESt API for groupService.create")
    const group = {_id: null, roles : [
        ]} as Group ;
    return group
  }
  ...
  //     const group = JSON.parse(p.stdout) as Group;
  //     return group;
  //   }
  ...
  //   appConfig = (JSON.parse(await fs.readFile(`/tangerine/groups/${groupId}/client/app-config.defaults.json`, 'utf8')) as any);
